...
- `build_data.py` — items.json → `data/` (프론트 로딩 분할). **items.json은 안 건드린다.**
  ...
  잘린 미리보기로는 그 계산이 불가능하고, 답이 페이지 로드마다 달라지지도 않기 때문이다:
    · `themes` — 8종 테마 매칭. **THEMES 정규식을 index.html에서 읽어와** 쓴다(복붙 금지,
      드리프트 원천 차단). 추출 실패 시 빌드가 죽는다.
    · `ents` — 카드가 언급한 엔티티(엔티티 페이지 필터의 근거). buildEntityMatcher와 동일 로직.
    · `_mk` — 주 종목(‘그 후 수익률’ 배지용). 필드명이 `_mk`인 건 itemMainKey()가
      `item._mk`가 있으면 그대로 반환하기 때문 — 프론트 수정 없이 그대로 먹는다.
...
- `items.json` (루트) — 발행된 카드 전체. scout만 쓴다(발행 규칙 참조).
  **★ 프론트는 이 파일을 더 이상 첫 로드에 받지 않는다 (2026-07-25).** 1.33MB(gzip 535KB)를
  전부 받아야 첫 카드가 그려지던 구조라, 모바일 4G에서 본문이 12초 뒤에 떴다.
  `items.json`은 발행자의 단일 진실 출처로 그대로 두고, `scripts/build_data.py`가
  아래 `data/`를 파생 생성한다. items.json 스키마는 바뀌지 않았다.
- `data/` (Actions 생성물, 사람이 직접 고치지 않는다)
  - `core.json` — **유일한 차단 페치.** 전체 153건의 목록·카드 필드 + gist를 150자
    미리보기로 자른 것 + 3개 언어. gzip 257KB (기존의 48%).
  - `gist.<lang>.<n>.json` — 전체 요약 원문, 최신순 24건씩 청크. 첫 페인트 **후**에
    받아서 이미 그려진 `<p class="gist">`의 텍스트만 갈아끼운다(재렌더 없음).
  - `elong.json` — 엔티티 longDesc 182건(180KB). 엔티티 페이지를 처음 열 때만 받는다.
  - `manifest.json` — 청크 수·gen 해시.
  - **폴백**: `data/core.json`이 없으면 프론트가 자동으로 `items.json`을 받는다.
    그래서 코드를 먼저 배포하고 Actions가 `data/`를 만드는 순서가 안전하다(검증 완료).
...
## ★ 반대편(the other side) — 카드가 스스로 반론을 들고 다닌다 (2026-07-25 신설)

한쪽 논지만 읽은 독자는 그 논지를 들고 나간다. 그래서 **어느 종목에 방향성을 낸 카드에는,
같은 종목을 반대로 본 가장 가까운 카드를 카드 안에 붙인다.**

- 짝짓기는 발행 시점에 `scripts/build_data.py`의 `pick_opposites()`가 정한다.
  프론트는 빌드가 써준 것만 그린다 — 런타임에 추론하지 않는다.
- 조건(전부 만족해야 함): ① 양쪽 다 **같은 회사를 명시**(cover.label 또는 tags) ②
  stance가 서로 반대(bull↔bear) ③ 45일 이내 ④ 한 글이 남의 반대편으로 쓰이는 건 최대 3회.
  카드당 최대 2건, 가능하면 서로 다른 저자.
- **틀린 짝은 없는 것보다 나쁘다.** 그래서 본문 스캔으로 종목을 추측하지 않는다.
  (ETF 자금유입 매크로 글이 인텔을 한 번 언급했다는 이유로 ‘인텔 글’이 되어 진짜 인텔
  분석의 반대편으로 붙은 사례가 실제로 나왔다.)
- 태그는 **별칭·티커까지 해석**한다. `NVDA` 태그를 못 알아보면 엔비디아에 약세인 글이
  두 번째 태그인 AMD로 잡혀서, **AMD 강세글의 반대편으로 붙는다** — 실제로는 같은 편인데.
  (`ticker: "nvda.us"` → `nvda` 로 정규화해서 매칭)
- 현재 153건 중 15건이 반대편을 갖는다. 적게 나오는 게 정상이다.
- UI: `.opp` 블록, ‘왜 중요한가’ 바로 밑(v83 order:35). 문구는 STRINGS의
  `oppLabel`/`oppNote`(3개국어).

**같은 패턴의 형제 — 이전 판단(priors, 2026-07-30 신설).** `build_data.pick_priors()`가
같은 저자·같은 명시 종목(explicit_key)·180일 이내의 이전 카드 최대 2건을 짝짓고,
`build_pages.py`의 `prior_block()`이 /p/ 3개 언어 페이지에 `.dlt` 블록("같은 종목, 이
필진의 이전 글")으로 렌더한다. 반대편과 같은 이유로 본문 스캔 추측 금지 — explicit_key
없으면 안 붙는다(현재 207건 중 31건, 적게 나오는 게 정상). 앱(index.html)에는 아직 미노출.
...
배포는 GitHub 웹 "Upload files"로 한다(샌드박스에 push 자격증명 없음). 이 경로는
**파일을 통째로 교체**한다. 오래된 사본을 올리면 그 사이 다른 창이 올린 커밋이
**충돌 없이, 흔적 없이** 사라진다. git이 안 막아준다.

2026-07-25에 실제로 한 세션이 `8399953`에서 클론해 한 시간 작업한 뒤, 그 사이 다른 창이
올린 `c38f7f5`(+103/-4)·`d80c1cc`(+21/-1)를 덮어쓰기 직전까지 갔다. 커밋 로그를 눈으로
확인해서 겨우 잡았다. 그래서 두 겹으로 막는다.

### 1겹 — 올리기 전에 (예방, 반드시)

```bash
python3 scripts/deploy_guard.py index.html sw.js        # 올릴 파일들
```
클론한 뒤 원격에서 그 파일이 움직였는지 본다. 움직였으면 **어떤 커밋이 몇 줄 넣었는지**와
rebase 명령을 찍고 종료코드 2로 죽는다. 안 움직였어도 최근 45분 내 다른 커밋이 있으면
"다른 창이 작업 중일 수 있다"고 경고한다.

rebase 한 뒤에는:
```bash
python3 scripts/deploy_guard.py --verify index.html sw.js
```
**rebase 성공 ≠ 상대 작업 보존**이다. 올라가는 건 rebase 결과가 아니라 그 파일의 바이트다.
그래서 상대 커밋이 추가한 줄이 지금 파일에 실제로 남아 있는지 한 줄씩 대조한다.
(사전검사가 베이스를 `.git/deploy_guard_base`에 적어두므로 범위가 정확하다. 이 파일이
없으면 최근 24시간으로 대신하는데, 그때는 자기 옛 커밋도 걸려 오탐이 난다 — 사전검사를
먼저 돌릴 것.)

### 2겹 — 올린 뒤에 (탐지, 자동)

`.github/workflows/clobber-guard.yml` → `scripts/clobber_check.py`.
푸시가 **최근 24시간 안에 다른 커밋이 추가한 줄을 지웠는지** 본다. 그게 오래된 사본을
덮어썼을 때의 지문이다. 걸리면 **실행 실패 + 이슈 자동 생성**.
- 오탐 억제: 공백·중괄호·짧은 줄 무시, 한 커밋에서 3줄 미만이면 무시, 봇 커밋은 제외
- 일부러 되돌리는 거면 커밋 메시지에 `[clobber-ok]`

⚠ 이 두 겹은 `claude/WORK-LOCK.md`(락)를 **대체하지 않는다.** 락은 애초에 둘이 같은 파일을
건드리지 않게 하는 장치고, 이건 락을 안 지킨 창이 있어도 피해를 막는 장치다. 둘 다 한다.
...
- **bilello_x(EN, rss.app X @charliebilello) — 같은 저자의 두 번째 수집 경로 (2026-08-04 추가).**
...
워커(`worker/index.js` /notify, `sent: res.ok`)가 이를 `sent:false` + HTTP 502로
반환하는데, `notify_followers.py`가 그 502를 `HTTPError`로 받아 `sys.exit(1)` 해서
run 전체가 Failure가 됐다. 팔로워가 거의 없는 현재 상태에선 릴레이가 멀쩡해도 사실상
항상 실패로 찍혔던 것(= "성공 확인된 적 없음"의 정체). 수정: `notify_followers.py`가
"구독자 0명" 응답(`_no_recipients()`: "all included players are not subscribed",
`"recipients":0` 등)을 정상 no-op으로 건너뛰고, 인증/시크릿 오류 같은 진짜 실패만
exit 1로 남긴다. 이제 팔로워 0명이면 run은 GREEN + job summary에 "구독자 0명(발송 없음)"
으로, 실제 구독자가 있으면 발송 후 "✅ sent"로 구분 표시된다. (워커가 no-recipients를
502 대신 200으로 주도록 고치는 건 선택적 후속 — 현재 수정만으로 릴레이는 정상 판정됨.)
