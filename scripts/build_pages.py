...
.xreal-slot iframe.x-direct{display:block;width:100%;height:380px;border:0;border-radius:12px;background:#fff}
...
(function(){
  ...
  function direct(host, id){
    var slot = host.querySelector('.xreal-slot');
    if (!slot) return;
    slot.innerHTML = '';
    var frame = document.createElement('iframe');
    frame.className = 'x-direct';
    frame.title = 'X post';
    frame.loading = 'eager';
    frame.scrolling = 'no';
    frame.referrerPolicy = 'strict-origin-when-cross-origin';
    frame.src = 'https://platform.twitter.com/embed/Tweet.html?id=' + encodeURIComponent(id) + '&dnt=true&lang=en';
    frame.onload = function(){ host.classList.add('x-on'); };
    frame.onerror = function(){ frame.remove(); };
  ...
  sc.onload = function(){
    if (!window.twttr || !twttr.widgets || !twttr.widgets.createTweet){
      Array.prototype.forEach.call(hosts, function(host){ direct(host, host.getAttribute('data-xid')); });
      return;
    }
    Array.prototype.forEach.call(hosts, function(host){
      ...
      try {
        ...
        }).catch(function(){ direct(host, id); });
      } catch (e) { direct(host, id); }
    });
  };
  sc.onerror = function(){
    Array.prototype.forEach.call(hosts, function(host){ direct(host, host.getAttribute('data-xid')); });
