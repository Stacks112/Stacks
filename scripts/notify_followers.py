...
GRADE_TAG = "daily"
GRADE_TITLE = "🎯 예측 채점"
# 회차당 채점 알림 상한. grading.md [2-A]의 회차당 판정 상한이 10건이라 그에
# 맞춘다. 옛 값 3은 "소급 정리 회차에 판정이 몰리면 알림이 소음이 된다"는
# 이유였는데, 실제로는 dedup 이 status 전이 감지 하나뿐이라 상한을 넘겨
# 생략된 건은 다음 회차 diff 에도 잡히지 않아 영영 발송되지 않았다
# (2026-08-05, 채점 10건 중 7건이 이렇게 유실). 소음보다 유실이 나쁘다고
# 판단해 10으로 올렸다.
MAX_GRADED_PER_RUN = 10
