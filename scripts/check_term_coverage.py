...
WATCHLIST = [
    ...
    "테스트베드", "램프업", "캐파", "생산능력", "가동률", "출하가", "고정거래가격",
...
def strip_markers(text):
    """마커 줄(REF의 URL·매체명, IMG 캡션 출처 등)은 검사 대상이 아니다."""
    return "\n".join(
...
UNIT_TOKEN = re.compile(r"^(?:\d+(?:Gb|GB|Tb|TB|Mb|MB|mm|W|GW|MW|kW|Hz|GHz|MHz|K|D|G|X|x)|H[12]|Gb|GB|TB|MB|Gbps|MHz|GHz)$")
