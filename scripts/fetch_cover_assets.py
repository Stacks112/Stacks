...
카드의 주 종목(entities의 ticker)의 **최근 1개월 종가 시계열**을 워커의
`/quote` 엔드포인트에서 받아 스파크라인 + 등락률 히어로 숫자로 그린다.

- 사실 근거가 확실하다 (LLM이 추출한 숫자가 아니라 실제 시세)
- 기사와 관련이 있다 (그 기사가 다루는 회사)
- 독자에게 실제로 쓸모가 있다 ("그래서 이 회사 주가는 어땠나")
...
4. 마크: 선 2px(2x에서 4px), 끝점 마커 8px 이상, 기준선은 점선 헤어라인,
   그리드 없음(스파크라인), 점마다 숫자 찍지 않음.
5. 상호작용: 정적 PNG라 해당 없음.
6. 접근성: 시리즈 1개라 범례 불필요. 등락은 색 + 화살표 + 부호 3중 인코딩.
7. 렌더 후 눈으로 확인 — 아래 `_selftest()`가 up/down/flat 3종을 뽑는다.
...
def render_quote_cover(out_path, entity, ticker, closes, dates=None):
    ...
    # 기간 시작가 기준 최소 ±2.5% 밴드를 보장해 작은 변동은 작게 보이게 한다.
    floor_lo, floor_hi = first * 0.975, first * 1.025
    ...
    # 기준선(기간 시작가) — 등락의 기준점. 점선은 안티패턴(노이즈·"예측"으로 읽힘)이라
    # 면에서 한 톤 내려간 실선 헤어라인으로 그린다.
    by = cy1 - (first - lo) / rng * (cy1 - cy0)
...
def make_data_cover(item, out_path, entities=None):
    """사다리 C: 카드의 주 종목 최근 1개월 종가로 스파크라인 커버를 그린다.
    ...
    if not ticker:
        return False
    try:
        url = f"{WORKER}/quote?s={ticker}&r=1mo"
        ...
        if not closes or len(closes) < 5:
            return False
        return render_quote_cover(out_path, name, ticker, closes, dates)
...
def process(item, entities):
    ...
    """
    iid = item.get("id")
    if not iid or item.get("coverArt"):
        return None

    out_rel = f"{COVER_DIR}/{iid}.png"
    if os.path.exists(out_rel):
        return {"coverArt": out_rel, "coverKind": item.get("coverKind") or "doc"}
    ...
    try:
        if make_data_cover(item, out_rel, entities):
            log(f"  [C] {iid}: 자체 생성 차트 커버")
            return {"coverArt": out_rel, "coverKind": "data"}
    except Exception as e:
        log(f"  [warn C] {iid}: {e}")
