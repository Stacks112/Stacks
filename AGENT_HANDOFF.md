...
**CI**: 각 회차 마지막 커밋에서 Feed detail regression ✅ (`aace27d` 는 아래 4번 사유로 ❌ 였고 `e66fd97` 에서 초록 복귀).

### 1. v90 — 허브를 카드 그리드에서 정렬 가능한 랭킹 표로

`upgradeHubCards` 가 `.inv-hub-table` 을 만든다. 열: 투자자 / 공개 평가액 / 보유 종목 / 상위 5종목 / 회전율 / 상위 보유. 기본 정렬은 공개 평가액 내림차순.
- 값 없는 항목은 **정렬 방향과 무관하게 항상 뒤로** 보낸다. `-Infinity` 치환은 내림차순에서만 뒤로 가므로 쓰지 않고 `null` 여부를 직접 분기했다(`hubCompareRows`).
- 죽은 스파크라인 제거: 표로 바뀌어 표시되지도 않는데 `investorCardFillSpark` 가 **허브 진입마다 `/quote` 194건**(투자자 수 × 티커 최대 5)을 유발하고 있었다. `investorCardHydrate` 에서 호출을 끊고 함수 자체는 남겼다(다른 화면 재사용 여지).

### 2. v90 — 비교 CTA 3중 노출 정리

데스크톱 허브에서 `.inv-compare-go`(인라인) · `.inv-bar-go`(하단 고정) · `.inv-rail-compare`(레일) **셋이 동시에** 떠 있었다.
- 레일 버튼은 **허브에서만** 제거. 개별 투자자/비교 화면에서는 레일이 유일한 CTA 라 그대로 둔다. 선택 칩은 무조건 남겨 `.inv-rail-selection` 이 비지 않게 했다(테스트가 가시성을 본다).
- 하단 바는 `IntersectionObserver` 로 인라인 CTA 가 화면에 있으면 숨긴다.
  ⚠ **기본값은 반드시 "보임"** 이다. IO 가 없거나 콜백이 안 오면 바가 그냥 보이는 v89 이전 동작으로 남는다. 반대로 설계하면 비교 진입로가 사라진다.

### 3. 페인트 게이팅 함정 — 네 번째 사례: IntersectionObserver

이 프로젝트는 같은 함정을 이미 세 번 밟았다(v82 스파크라인, v85 비교 렌더, v88 엔티티 로더). **IO 도 똑같이 걸린다.**
`visibilityState: "hidden"` 탭에서 `requestAnimationFrame` 이 2초 안에 안 돌고, 직접 만든 IO 콜백도 안 깨어난다(라이브에서 실측). 그래서 하단 바 자동 숨김은 이 환경에서 **검증 불가**이고, fail-open 설계로만 안전을 담보한다.

### 4. 항상 빨간 회귀 가드 복구 (`e66fd97`)

feed-detail 5개 테스트가 **`6e563c3b7` 이후 결정적으로 실패**하고 있었다. v90 탓이 아님을 대조군 실험으로 확인했다(v90 직전 `0394966` 6 failed / v90 후 `aace27d` 5 failed).
원인: `eca0365` 자동발행 카드가 피드 1번이 됐는데 이 카드의 **유일한 엔티티 언급이 접힌 `<details class="srcs-fold">` 안에만** 있어 DOM 에는 있지만 bounding rect 이 0x0 이다.
- 고친 것은 **픽스처 선택 기준**이지 단언이 아니다. `toBeVisible()` 은 그대로 두고 `firstLinkedCardId` 가 "본문에 실제로 보이는 ent-link" 를 가진 카드만 고르게 했다(닫힌 `<details>` 조상 배제 + rect 0 배제).
- **자격 있는 카드가 없으면 조용히 넘어가지 않고 명시적으로 실패**하게 했다. 가드가 무력화되는 건 빨간불보다 나쁘다.
- 엔티티 링크가 점진적으로 붙는다는 걸 발견해 스캔을 `toPass({timeout:10_000})` 로 감쌌다. 한 번만 훑으면 초기 렌더 상태를 붙잡는다.

### 5. v91 — 비교 요약을 지표=행 표로 전치

투자자 4명 × 라벨 9개 = 라벨 36회 반복이던 카드 묶음을 `<table class="inv-compare-summary-table">` 하나로 바꿨다. 열 헤더 `<th class="inv-compare-col">` 안에 기존 `.inv-compare-card-head` 버튼을 **그대로** 넣어 클릭 경로와 테스트를 보존했다. 셀은 컨테이너 엘리먼트가 아니라 **slug 로 주소지정**(`model.cols` / `model.perf`)한다 — 열에는 더 이상 감싸는 엘리먼트가 없다.

**우위 하이라이트는 9개 중 4개 행에만 붙인다** (`since`/`3m`/`1y`/`vsSpy`). 나머지는 방향이 정당하지 않다:
- `13F 기준`(날짜) · `공개 평가액`(펀드 규모) → 순위 대상이 아님
- `보유 종목 수` · `상위 5종목` · `회전율` → **방향이 진짜 모호하다.** 회전율이 높은 게 기민한 건지 잦은 건지, 집중이 확신인지 위험인지 코드에도 COPY 에도 사이트의 입장이 없다. 여기에 승자를 붙이면 **없는 입장을 만들어내는 것**이다. 다음 사람도 이 선을 넘지 마라.
- 유효값이 2개 미만이거나 **동점이면 아무 셀도 칠하지 않는다**(`updateRowWinner`, 값 도착마다 재계산해 비동기 순서에 안전).

### 6. v92 — 상시 노출 히어로 수치 · 면책 1곳으로 · 미분류 비중 명시

- **히어로**: "그대로 들고 있었다면" 섹션에 **평상시 보이는 숫자가 아예 없었다.** 제목이 16px 로 다른 섹션과 같고 CSS 전체에 18px 초과가 없었으며, 수익률은 **호버·드래그해야만** 나왔다. 투자자별 서명된 수익률을 28px(모바일 21px)로 상시 노출한다. 값은 `paintCompareGraph` 가 **이미 그린 series 에서** 가져와 선과 절대 어긋나지 않는다. 기간 탭·벤치마크 토글·드래그 선택·해제 모두에 연동된다. 데이터 없으면 `0.0%` 가 아니라 `holdGraphNoPrice`.
- **면책**: "옵션 제외" 가 차트 각주와 면책 문단 두 곳에 중복이었다(레일 카드는 옵션 단어가 없어 손대지 않음). 차트 각주에서 중복 접두어만 빼고 **`제외 항목 전체 보기 →` 포인터**를 붙여 면책 블록(`#inv-compare-disc`)으로 스크롤시킨다. 정보를 지운 게 아니다.
  ⚠ 진짜 `<a href="#inv-compare-disc">` 를 쓰면 **비교 화면이 통째로 날아간다.** 이 SPA 의 popstate 핸들러가 state 가 null 인 히스토리 항목(평범한 프래그먼트 링크가 push 하는 것)을 "인식 못 함 → 오버레이 닫기" 로 처리하고 주소를 `/` 로 되돌린다. 스크롤만 하는 `<button>` 으로 라우터를 우회했다.
- **미분류 비중**: 섹터 표에 `미분류` 행을 추가하고, 투자자별로 "공개 평가액의 X%만 분류된 결과입니다 · 나머지 Y%는 미분류입니다" 를 명시한다. `covered_pct` 가 null 이면 `100% 미분류` 같은 거짓말 대신 커버리지 불명으로 표기. 커버리지 50% 미만은 경고색.
  라이브 실측: **DRUCK 11.9% 분류 / 88.1% 미분류** — 12% 로 그린 섹터 구성을 전부인 양 보여주던 문제가 이제 화면에 드러난다.

### 검증

- 4개 회차 전부 **커밋 전 base SHA-256 확인 → dispatch → 커밋 후 바이트 완전 일치** 확인. 최종: `index.html` `d23c0b25…`(995,630) · `investor-compare.js` `06e94329…`(79,328) · `investor-compare.css` `ff628d95…`(24,682) · `feed-detail.spec.mjs` `144a3dd8…`(36,293).
- 로컬 스위트: v91 27/27, v92 28/29(1건은 아래 기존 플레이크). CI 는 각 회차 마지막 커밋에서 초록.
- 라이브(v92): 허브 표 16행 · 레일 CTA 0개 · 스파크라인 DOM 0개 · 요약표 4열 13행 · **우위 표시가 `since`/`vsSpy` 에만, 모호한 행에 0개** · 히어로 4개 28px · 각주 1줄 + 포인터 · 미분류 4줄(3건 경고색).

### 남은 위험 / 다음 사람에게

1. **`clickHeadline` 픽스처 결함 (미수정, 우선순위 높음).** CI 에서 `headline has no clickable non-entity spot` 로 간헐 실패한다. 헤드라인 **세로 중앙 한 점만** 찍어보고 실패하는데, 여러 줄로 감긴 헤드라인의 그 줄만 엔티티로 덮이면 바로 던진다. 방금 고친 `firstLinkedCardId` 와 **같은 계열의 버그**다. 제안: `toPass()` 로 감싸고 `getClientRects()` 로 줄마다 y 를 훑을 것. 로컬 35회 재현 0회라 정확한 트리거는 **확인 불가**.
2. **기존 플레이크 2건** (v90 이전부터 존재, 대조군으로 확인): `13F deep link renders the investor detail` 의 `#feedList .inv-table` 이중 렌더(약 1/5), `desktop skew rows expand source detail` (약 1/5). 이중 렌더의 실제 메커니즘은 **확인 불가**.
3. **`compute_sector_alloc` 분모 의심** (`scripts/fetch_13f.py:815`). 바로 위 티커 커버리지 함수와 달리 **PUT/CALL 행을 걸러내지 않는다.** 상류 `merged` 가 이미 걸러졌는지는 확인 못 했다. 사실이면 커버리지 %가 옵션으로 희석된다.
4. **반올림 표시 오차**: `pct(cov,1)` 과 `pct(1-cov,1)` 을 따로 반올림해 합이 100.1% 로 보이는 경우가 있다(라이브 ASCHEN 75.8 + 24.3). 표시만의 문제지만 눈에 띈다.
5. COPY 키 `changes`("분기 변화")는 정의만 되고 아무 데서도 쓰이지 않는다.

### 배포 도구 함정 (다음 회차에 그대로 재발한다)

- **CodeMirror 오프셋은 UTF-16 코드유닛이다.** `index.html` 에 서로게이트 페어가 29개 있어 파이썬 코드포인트 기준으로 계산하면 **29 만큼 밀린다**(코드포인트 895,636 ↔ UTF-16 895,665). 커밋 전 target 해시 검증이 이걸 잡아냈다. `assets/*.js|css` 는 서로게이트가 없어 무관.
- **크롬 확장이 큰 페이로드에서 끊긴다.** 3KB 넘는 `javascript_tool` 호출이 `Couldn't determine which page this action targets` 로 실패하고, 재시도 중 탭이 통째로 사라져 `window.*` 누적이 날아갔다. **base64 로 인코딩해 1.5KB 씩 `localStorage` 에 누적**한 뒤 마지막에 디코드·dispatch 하면 탭이 죽어도 이어갈 수 있다.
- **회차 중간 커밋은 CI 가 빨간 게 정상이다** (CSS 만 들어가고 JS 가 아직인 상태). 다만 v91 때 확장이 끊겨 **CSS 만 커밋된 채 한동안 방치**됐고, 그 CSS 가 `.inv-compare-card`/`.inv-compare-metrics` 규칙을 지워 비교 화면 스타일이 깨진 상태로 노출됐다. **자산 파일은 반드시 한 번에 몰아서 넣어라.**
...
13F 종목명이 SEC 원문 그대로 영문 대문자였다(`AMAZON COM INC`, `TAIWAN SEMICONDUCTOR M…`).
헬퍼 3개(`invStripSuffix` · `invEntityName` · `invDisplayName`)를 만들어 막대 · 매수매도 상위 · 상위25 표 **세 곳**에 적용했다.

- **접미사 정리(412행 전부)**: `INC`(207) · `CORP`(83) · `LTD`(21) 등 끝에 붙는 법인 접미사를 뗀다.
- **엔티티 연결분(26.2%, 108행)은 그 언어 이름으로**: 한국어 화면 `애플` · `구글` · `뱅크오브아메리카`, 영어 화면 `Apple` · `Google` · `BofA`.
- **막대의 `title` 툴팁은 SEC 원문을 유지**해 원자료 접근성을 잃지 않게 했다.

**★ 약어 브랜드는 일부러 영문을 유지한다.** 별칭의 한글이 `에이에스엠엘` · `티에스엠씨` · `에이엠디` 같은 음차라 그대로 쓰면 오히려 읽기 나빠진다.
`aliases` 의 첫 라틴 별칭이 5자 이하 전부 대문자면 약어로 보고 영문을 쓴다(TSMC · ASML · AMD · IREN 확인).

### 완전한 한글화는 지금 불가능하다 (다음 세션이 다시 시도하지 않도록)

- `company` 엔티티 **197개 전부 `name` 필드가 없다.** 한글 표기는 `aliases` 배열 안에 순서 보장 없이 섞여 있다.
- 기존 `entName()`(4650)은 `e.name` 이 없으면 `dispName(key)` 로 빠져 **키 자체(영문 대문자)를 돌려준다.** 회사명에는 도움이 안 된다.
- 13F 412행 중 `entity_key` 가 있는 건 **26.2%뿐**이다. 투자자별 편차가 크다(시추에이셔널 57% · 버크셔 10% · 아이칸 0%).
- 그래서 "첫 번째 한글 별칭" 휴리스틱을 쓰되 **13F 와 매칭되는 28개 키 전부를 눈으로 검증**했다(반례 0). 197개 전체에서 성립한다는 보장은 **확인 불가**.

### 배포 전 전수 시뮬레이션이 결함 3건을 잡았다

412행 × 3언어를 미리 계산해 본 덕에 다음이 배포 전에 걸렸다.

1. `Deere & Co` -> `Deere &` — 앰퍼샌드가 대롱대롱 남았다(JPMorgan Chase & Co 도 동일). 꼬리 정리 추가.
2. `VANECK ETF TRUST` -> `VANECK ETF` — 펀드·REIT 에서 TRUST 는 구조를 나타내는 핵심 단어다. 접미사 목록에서 뺐다.
3. **아마존만 `aliases[0]` 이 한글**이라 영어 화면에 `아마존` 이 뜰 뻔했다. 첫 **라틴** 별칭을 찾도록 고쳤다.

효과: 이름 평균 길이 17.3자 -> 11.3자, 22자 초과(잘리는) 행 88 -> 41. 빈 문자열·2자 미만·원본보다 길어짐 **0건**.
같은 투자자 안에서 이름이 겹치는 쌍은 17 -> 17 로 **늘지 않았다**(대부분 보통주 행 + 옵션 행이라 원래 같았고 PUT/CALL 배지로 구분된다).

### ★★ 이번 회차 최대 발견 — 엔티티 로더가 백그라운드 탭에서 아예 안 돈다

라이브 검증에서 한글 이름이 안 나와 원인을 팠더니, `ENTITIES` 가 **0개**였다.

| 시점 | ENTITIES | `.ent-link` | `/data/entities.json` 요청 |
|---|---|---|---|
| hidden 탭, 로드 후 110초 | **0개** | **0개** | **없음** |
| 스크린샷 1장(강제 페인트) 직후 | **692개** | **171개** | 있음 |

**탭은 그때도 계속 `hidden` 이었다.** 즉 보조 데이터 로더(entities/events)가 첫 페인트에 물려 있어 비가시 탭에서는 영원히 안 뜬다.
v85(비교 화면 rAF) · v82(스파크라인) 에 이은 **세 번째 같은 계열**이다.

**실사용자(가시 탭)에게는 정상**이지만, 이것이 뜻하는 바가 크다:
- **백그라운드 탭에서 라이브를 검증하면 색인·엔티티가 통째로 없는 화면을 보게 된다.** 과거 세션들이 반복해서 헷갈린 원인일 가능성이 높다.
- 검증 전에 `Object.keys(ENTITIES).length` 를 먼저 재라. 0이면 스크린샷 한 장으로 페인트를 유도한 뒤 다시 봐라.

### 남은 한계 — 콜드 진입에서는 한글 이름이 안 나온다

엔티티가 **렌더 이후에 도착**하고 재렌더가 없다. 그래서:

- `#investor-berkshire` 로 **직접 진입**하면 엔티티 도착 전에 그려져 영문(접미사만 정리된)으로 남는다.
- 홈을 거쳐 **앱 안에서 이동**하면 정상이다. 라이브 실측으로 `애플` · `뱅크오브아메리카` · `구글` 확인.

즉 경쟁 상태다. 폴백이 여전히 개선된 영문이라 깨지지는 않지만 **결정적이지 않다.**
고치려면 보조 로더 완료 콜백(14097 부근)에서 13F 뷰가 열려 있을 때 이름만 다시 칠하는 경로가 필요하다.
⚠ 그 콜백은 2026-08-10 회차가 "비교 화면 복귀" 버그로 `!invViewActive()` 게이트를 건 자리다. 건드릴 때 그 버그를 되살리지 마라.

### 검증

- `node --check` 통과. 패치본에서 함수를 추출해 412행 × 3언어를 재계산 -> 사전 시뮬레이션과 **1,236개 값 전원 일치**.
- CRLF 15,668 -> 15,702(추가 34줄과 일치, LF 단독 0). 커밋 전후 SHA-256 대조로 **바이트 완전 일치**(`c166e3d9…`, 994,508바이트).
- 라이브: BUILD v88, 접미사 정리 전 화면 확인, 툴팁에 SEC 원문 유지 확인, 웜 이동 시 한글 이름 확인.
- CI: **`Feed detail regression` 이 push 에서 1회 실패**했다. 원인은 `strict mode violation: .inv-value-card .inv-compare-period[data-period="1d"] resolved to 2 elements` 로,
  **이름 변경과 무관한 이중 렌더 flaky** 다(25 passed / 1 failed). 같은 main 에서 `workflow_dispatch` 재실행 -> **success**.
  ⚠ **v85 회차의 "serviceWorkers 차단으로 flaky 가 사라졌다" 는 추정은 너무 낙관적이었다.** CI 에서도 여전히 난다. 이중 렌더 경합은 미해결로 남아 있다.

### 다음 회차가 알아야 할 것

- **이중 렌더 경합이 진짜 미해결 과제다.** `.inv-value-card` 가 순간적으로 2개가 된다 = `renderInvestors` 가 두 번 그린다. 재현이 산발적이라 잡기 어렵지만 CI 를 계속 흔든다.
- 콜드 진입 한글 이름(위 「남은 한계」)을 마저 하려면 보조 로더 완료 시점 재칠하기.
- `build_pages.py` 가 만드는 개별 종목 페이지의 13F 보유자 목록에는 이 이름 로직이 **적용되지 않았다**(클라이언트 전용 변경).
- 접미사 목록에 `ETF` 는 넣지 않았다. `STATE STREET SPDR S&P OIL & GAS ... ETF`(60자)는 여전히 최장이다.

---

## 2026-08-11 Cowork(Opus) — 옵션 포지션이 주식 보유로 읽히던 문제 (v87)

**커밋**: `de28455`(index.html, +21/−3, 단일 파일). BUILD v86 -> v87. 에셋 무변경.
**CI**: Feed detail regression ✅ · Clobber guard ✅ · Email render guard ✅ · Mobile calendar regression ✅ · Watch auto-publish ✅.
(`pages build` 은 9초 뒤 들어온 자동발행 커밋 `6b402c6` 에 의해 cancelled 됐고 그 커밋의 빌드가 success 했다. 정상 동작이니 오탐하지 말 것.)

### 무엇이 틀리게 보였나 (라이브 실측)

투자자 상세의 비중 막대와 「매수 상위」 목록에 옵션 포지션이 **일반 주식과 똑같은 색·똑같은 스케일**로 섞여 그려지고 있었다.
구분 수단은 이름 옆의 작은 `PUT`/`CALL` 배지 하나뿐이었다.

마이클 버리(사이온) 페이지 실측, 수정 전:

| 순위 | 종목 | 종류 | 비중 | 막대 색 |
|---|---|---|---|---|
| 1 | 팔란티어 | **PUT** | 66.0% | `rgb(124,147,255)` |
| 2 | 엔비디아 | **PUT** | 13.5% | `rgb(124,147,255)` |
| 3 | 화이자 | CALL | 11.1% | `rgb(124,147,255)` |
| 4 | 핼리버튼 | CALL | 4.5% | `rgb(124,147,255)` |
| 5 | 몰리나 헬스케어 | 주식 | 1.7% | `rgb(124,147,255)` |

**진짜 주식 1위는 5번째(1.7%)인데 화면에서 가장 긴 막대는 하락 베팅이었다.**
「매수 상위」에는 `PALANTIR TECHNOLOGIES INC PUT 66.04% 신규` 로 올라와 "팔란티어를 새로 샀다" 로 읽혔다.

**데이터 규모**(`portfolios.json` 실측): 보유 행 412개 중 PUT 21 · CALL 23.
옵션 보유 투자자는 16명 중 5명이고, 그중 **버리·시추에이셔널·소로스 3명은 옵션이 포트폴리오 비중 1위**다.
PUT 21행 중 18행이 상위 25 표에 노출된다.

**★ 이 원칙은 이미 사이트 안에 있었다.** `assets/investor-compare.js:96` 의 `holdings()` 와 `build_pages.py` 는
`put_call !== "PUT" && put_call !== "CALL"` 로 **옵션을 이미 제외**하고 계산한다. 상세 페이지 차트도 "옵션 제외" 라고 표기한다.
**투자자 상세의 막대·무버스 목록만 그 원칙이 안 걸려 있었다.**

### 고침 (june 결정: 감추지 말고 구분해서 계속 보여준다)

1. **막대 색 분리** — `inv-bar-fill-put`(`#E04438`) / `inv-bar-fill-call`(`#0E9F5E`) 클래스를 `h.put_call` 로 붙인다.
   기존 `.inv-opt-put`/`.inv-opt-call` 배지와 **같은 hex** 라 화면이 한 시스템으로 읽힌다. 다크 오버라이드는 안 넣었다(기존 배지도 없다).
2. **안내문 신설** — 옵션 보유가 있을 때만 막대 바로 아래에 `.inv-opt-note` 한 줄. ko/en/ja 3개국어(`invOptNote`).
   문구는 **의도를 단정하지 않는다**: 풋 매수가 반드시 하락 베팅인 것은 아니고 헤지일 수 있다. 그래서 "하락 베팅" 이 아니라
   "주식 보유가 아니라 옵션 포지션이고, 13F 금액은 계약이 커버하는 주식 가치라 실제 투입 금액과 다르다" 는 사실만 적었다.

**금액 자체도 부풀려져 있다는 점이 중요하다.** 13F 의 옵션 `value` 는 계약이 커버하는 기초자산 가치(노셔널)라
실제로 넣은 돈이 아니다. 66% 라는 크기 자체가 롱 주식과 같은 축에서 비교될 수 없는 숫자다. 안내문이 이걸 말한다.

### 검증

- `node --check` 통과. i18n 무결성을 node 로 실제 평가해 확인 — en/ko/ja 각 34키 -> 35키, `invOptNote` 3개 언어 전부 문자열, **기존 키 손실 0건**.
- CRLF 15,650 -> 15,668(추가 18줄과 정확히 일치, LF 단독 0).
- 커밋 전 에디터 doc 을 CRLF 환산해 origin `3977813e…` 와 대조, dispatch 후 `2a94555b…`(992,732바이트)가 로컬 검증본과 일치. 커밋 후 `api.github.com/contents` 로 재확인 — **바이트 완전 일치**.
- 회귀 스위트 2회: 26/26, 25/26(무관한 skew 테스트 1건, 단독 재실행 시 통과 = 기존 flaky).
- **라이브 실측**:
  - 버리(ko): PUT 막대 `rgb(224,68,56)` · CALL 막대 `rgb(14,159,94)` · 주식 막대 `rgb(124,147,255)` 그대로. 안내문 렌더·가시 확인, 위치는 `inv-bars -> note -> inv-movers-grid`
  - 소로스(en): 1위 SPY PUT 이 put 클래스로 전환, 옵션 막대 5개, 영어 안내문 정상
  - 사이온(ja): 옵션 막대 4개, 일본어 안내문 정상
  - **버크셔(옵션 0건): 막대 17개, 옵션 막대 0개, 안내문 미표시** — 조건부 렌더가 의도대로 동작

### 다음 회차가 알아야 할 것

- **「매수 상위」 목록의 문구 자체는 안 바꿨다.** 옵션 행도 여전히 종목명 + `PUT` 배지로 나온다.
  막대 색과 바로 위 안내문으로 맥락이 잡히지만, "매수 상위" 라는 제목 아래 풋이 있는 어색함은 남아 있다.
  더 손보려면 무버스 행에 "풋 매수" 같은 성격 문구를 넣는 방향이 있다. june 판단 필요.
- **상위 25 표는 손대지 않았다.** 표에는 막대가 없고 배지가 이미 붙어 있어 오해 소지가 상대적으로 작다고 봤다.
- 막대 이름의 `textContent` 가 `PALANTIR TECHNOLOGIES INCPUT` 처럼 **공백 없이 이어진다**(배지가 별도 span 이라 화면에는 `margin-left:5px` 로 정상 표시된다).
  v84 의 `버크셔 해서웨이INV:BUFFETT` 와 같은 계열이다. textContent 를 읽는 코드가 생기면 문제가 된다 — 지금은 무해.
- 옵션 배지 텍스트(`PUT`/`CALL`)는 3개국어 모두 영문 그대로다. 금융 용어라 관례일 수 있으나 의도인지 누락인지는 **확인 불가**.
...
Clobber guard 둘 다 **success**. 오늘 세 번째 배포(앞의 둘: `8530edb` 데스크톱 겹침, `07999bf`+`21ec503` 모바일 이름).

**문제**: 랭킹이 `pct` 내림차순인데 `pct`는 **출처 기준**(`max(sourceBull,sourceBear)/sourceDir`)이라
**출처 1곳이면 무조건 100%**다. 글 1건짜리가 1~5위를 독식하고, 출처 3곳·글 4건짜리가 67%로 꼴찌였다.
순위가 확신도가 아니라 **표본 부족을 정렬**하고 있었다.

**패치** (`index.html` 1곳 + `assets/v82.js` 2곳)
- `v83ThemeAttention` 정렬: `rankBucket -> pct -> dir` 에서
  `|sourceBull-sourceBear| desc -> sourceDir desc -> pct desc -> dir desc` 로 교체.
- `v83SkewShare(c)` 신설 — 출처 1곳이면 **퍼센트 미표시**, 2곳 이상이면 `2/3` 분수.
  히어로 칩·랭킹 행 양쪽, 데스크톱·모바일 공통(v82 가 이 함수를 재사용).

**설계 근거 (데이터로 두 번 방향을 바꿨다 — 다음 세션이 되돌리지 않도록 남긴다)**

1. **접기 방식은 폐기했다.** 처음 승인된 안은 "글 3건 미만은 하단 접기 그룹으로"였다.
   12주를 재현해 보니 **"글>=3 AND 출처>=2" 통과 행이 0개인 주가 7/12**, 0~1개인 주가 8/12.
   **7주는 상단이 통째로 빈 화면이 된다.** 기업은 이번 주 24개 후보 중 통과 0개.
2. **"글 N건 중 M건" 표기도 폐기했다.** `pct` 는 출처 축이라 글 축으로 쓰면 **화면에 거짓 숫자**가 나간다.
   12주 중 출처 기준 다수와 글 기준 다수가 **실제로 갈리는 사례 4건**(예: HUAWEI 07-28 주 —
   출처 기준 mix(1:1), 글 기준 bear(1:3)). 분수는 반드시 `max(sourceBull,sourceBear)/sourceDir`.
3. **1차 키를 `sourceDir` 로 하면 안 된다.** 그렇게 하면 **mix(50:50) 행이 상위로 올라온다** —
   2026-07-29~08-04 주에서 `AI capex`(2:2)가 **2위**였다. 제목이 「지금 쏠린 곳」인데 가장 안 쏠린
   항목이 2위인 건 개념이 깨진 것. 순 마진으로 바꾸니 같은 주 **7위(꼴찌)** 로 내려갔다.
   12주 전수에서 **mix 가 bull/bear 행을 앞지른 사례 0건.** 옛 `rankBucket` 의 mix 강등 효과는 보존된다.

**검증**: `node --check` 전부 통과. 순위변동 배지는 `now/prev` 가 같은 `v83ThemeAttention` 에서
나오므로 자동으로 새 기준 적용(6행 전수 산수 대조 일치). 모바일 390/360/340px **이름 잘림 0개 유지**
(`07999bf` 회귀 없음), 행 높이 82px 유지. 표본 풍부했던 07-21·07-28 주 재현도 이상 없음.
라이브(1311px): 랭킹에서 `%` 문자 **완전히 사라짐**, 출처 1곳 행은 `강세` 만 표시, 겹침 0건.
`index.html` sha256 `6260c1db…f8dfc670` 바이트 일치, `v82.js` sha8 `70f2d061` 일치.

**분석 스크립트**: `/mnt/user-data/outputs/skew-threshold-analysis.py` (12주 재현. 세션 종료 시 사라지므로
계속 쓰려면 레포 `scripts/` 나 `tests/` 로 옮길 것). 원자료 `skew-threshold-weekly-data.json`.

## ⚠ 조율 문제 — 같은 화면을 두 세션이 반대 방향으로 밀고 있다

오늘 내가 고친 코드가 **전부 어제(2026-08-10) 이 레포의 다른 세션(Codex 추정)이 만든 것**이다.
어제 `#skew` 한 화면에만 커밋 6개가 있었다:
`af08b38 clarify skew ranking` · `9c0053c add source original links` · `fca43be track source clicks` ·
`b34bacd clarify sample freshness` · `c0242a0 clarify source states` · `fbd3f89 finish source trust cues`.

오늘 내가 되돌린 것:
- `c0242a0` 이 `leanLb` 에 붙인 `sampleDetail` -> 제거(모바일 이름이 30px 로 뭉개진 원인)
- `c0242a0` 이 만든 `.v82-skew-source-toggle` -> 히트박스 추가로 수정
- `af08b38` 의 `rankBucket` 정렬 -> 순 마진 정렬로 교체

**되돌린 판단 자체는 실측 근거가 있다**(위 참조). 문제는 **저쪽이 이 사실을 모른다**는 것이다.
같은 의도("표본이 얇다는 걸 화면에 드러내자")를 양쪽이 서로 다른 방법으로 구현하다 충돌했다 —
저쪽은 **문자열을 더 붙이는 방향**, 이쪽은 **중복을 걷어내는 방향**.
다시 넣으면 무한 반복이다. **`#skew` 를 만지기 전에 이 절과 `claude/design-2026-08-11-skew-page-ux-audit.md`
를 먼저 읽을 것.**

**남은 것**
- `Feed detail regression` 워크플로가 2026-08-10 `af08b38` 이후 **계속 failure**(오늘 커밋 전부 포함).
  이번에도 내 변경이 깬 게 아니다. 회귀 가드가 죽어 있다.
- Clobber guard 이슈가 안 닫힌 채 쌓인다 — `#43` 은 조사 후 닫았지만 `#28~#36` 8건이 08-07~08-08부터 열려 있다.
- **하단 「일별 추이」는 글 기준 산식이라 상단 랭킹(출처 기준)과 같은 항목에 다른 숫자가 뜬다.**
  기존 프로덕션 상태이고 이번 범위 밖으로 뒀다. 고치려면 산식 통일이 필요하다.
- P1 잔여(근거 글 인라인 노출, 3중 중복 구조 통합)와 P2·P3 는
  `claude/design-2026-08-11-skew-page-ux-audit.md` 참조.

**안전 범위**: `items.json`, D1 큐, 자동 발행 워크플로, `scripts/` 는 건드리지 않았다.
...
전체 진단 후 june이 우선순위를 정해 버그성 항목부터 착수. 코딩은 서브에이전트(sonnet) 3회, 판단·커밋·검증은 본 세션.

**변경 파일**: `assets/investor-compare.js` (커밋 `a9fd834`, +22/-15), `index.html` (커밋 `b43d5c2` +420/-393, 이후 v83 캐시버스터 커밋 1건)

### 배포한 것

**1. 투자자 비교 선택을 FIFO 교체 방식으로 (investor-compare.js)**
- 4명을 채우면 나머지 12장 버튼이 `disabled`가 되어 화면 대부분이 죽어 보이던 동작 제거
- 5번째 선택 시 `selected.values().next().value`로 가장 오래된 항목을 자동 해제. Set의 삽입 순서 보존에 의존
- `MAX_COMPARE = 4` 상수 도입, 흩어진 매직넘버 4를 5곳 치환. `shortInitial`의 `.slice(0,2)`는 무관하므로 제외
- 안내 문구 `swapped` 신설(ko/en/ja). 한국어 조사(을/를)가 이름 받침에 따라 달라지는 문제를 피하려 "{out} → {in}" 화살표 구조 채택
- `max`, `fullCompare` 키는 이 파일에서 더 이상 참조되지 않으나 삭제하지 않음
- **라이브 검증 통과**: berkshire가 빠지고 appaloosa가 들어감, 문구 "최대 4명이라 오래된 선택을 교체했어요: 워런 버핏 → 데이비드 테퍼", disabled 버튼 0개, 레일 4/4

**2. 카드 스파크라인 프리로드 여유 + 폴백 (index.html, observeInvestorCards)**
- `rootMargin` "0px 0px" 에서 "200px 0px" 로. 카드 높이 205px이므로 약 1장 선행
- `visibilitychange` 리스너 추가: 탭이 다시 보이면 뷰포트 안 미수화 카드만 hydrate
- 3초 안전망 1회: IntersectionObserver 자체가 발화하지 않는 경우 대비
- 뷰포트 밖 카드는 건드리지 않음. `io.unobserve` + `__invHydrated` 가드로 카드당 실행 1회 불변
- IO 미지원 idle 폴백 경로는 바이트 그대로 유지
- 배경: 목록 카드 스파크라인은 `inv.snapshots`로 **동기 계산**한다. 상세 화면의 Q1Y/QR 시세 캐시와 다른 경로이므로 rootMargin 확대가 `/quote` 부하를 늘리지 않는다

**3. 캐시버스터 갱신 (index.html, v83)**
- 1번을 커밋했는데 `?v=6c3d74b2`가 구버전 해시라 브라우저가 이전 파일을 계속 받고 있었다. 배포 직후 disabled 12개가 그대로여서 발견
- `?v=6d4c877c`로 갱신. 규칙(`sha256(파일)[:8]`)은 구버전 파일 해시가 정확히 `6c3d74b2`임을 계산해 검증
- **교훈: assets/ 아래 파일을 고치면 index.html의 `?v=`를 반드시 같은 라운드에 갱신할 것**

### 진단했으나 배포하지 않은 것

**A. 비교 화면 4열 가로 잘림 (미해결, 다음 회차 최우선)**
- 데스크톱에서 `.inv-compare-table-wrap` 598/620 초과, 요약 카드가 228px가 아닌 141px로 압착
- 원인: `html.v83.inv-wide` 폭 확장 CSS는 완비돼 있는데 **켜는 JS가 없고 `renderFeed()`의 무조건 `remove`만 있다**
- 런타임에서 `inv-wide`를 켜보니 표 오버플로는 해소되나 **`#v83rail`이 absolute로 본문 위에 얹혀 상단 0~729px 구간을 가린다**. 목록의 "비중" 값, 상세/비교 차트 우측 약 310px이 덮임
- june 결정: inv-wide 대신 **셸 폭 재배분**. 13F에서만 좌측 네비 240에서 200, 레일 340에서 280으로 줄여 가운데를 600에서 750으로. 겹침이 원천적으로 없고 표 요구폭 620px도 충족
- 참고: `cal-wide`가 같은 패턴의 확립된 관용구다. `#railFoot` 숨김과 `.v83post-title` ellipsis는 이미 CSS에 반영돼 있고 런타임에서 의도대로 동작함을 확인

**B. 상세 헤더 티커 공백 누락 (원인 미확정)**
- "투자자 › 버크셔 해서웨이INV:BUFFETT"로 붙어서 렌더. 확대 스크린샷으로 육안 확인
- `investor-compare.css`에 `.series-head-name>.inv-ticker{margin-left:8px}`가 있고 computed로도 8px가 잡힌다
- 그런데 **데스크톱에서 실제로 보이는 헤더는 `.series-head-name`이 아니다**. 그 요소는 rect가 전부 0인 숨김 상태였다. 실제 렌더되는 헤더 요소를 특정하는 것이 다음 단계

**C. 그 밖의 UI 제안 (미착수)**
- 허브를 카드 리스트에서 정렬 가능한 랭킹 테이블로. 행 205px에서 40px대로, 16명이 한 화면. 평가액·회전율·집중도·공시후수익률 컬럼 정렬
- CTA 3중 제거(상단 배너, 우측 패널, 하단 바 중 하단만), 검색창 2개 중복 제거
- 비교표 전치: 지표를 행, 투자자를 열, 라벨 1회만, 행별 1등 하이라이트
- 상세: "그대로 들고 있었다면" 수치를 히어로로, 면책 5줄을 접이식 1개로, 막대 리스트와 테이블 통합, 섹터 "미분류 62%" 명시, KPI 색상 방향성 부여

### 남은 리스크

- `index.html` 커밋의 diff 813줄 중 **실제 내용 변경은 30줄이고 나머지 390줄은 LF에서 CRLF로의 줄바꿈 정규화**다. CodeMirror가 다수파 줄바꿈으로 직렬화한 결과. 내용 등가성은 patch의 add/del을 `\r` 제거 후 대조해 확인했다. 이제 파일 줄바꿈이 CRLF로 통일됐으니 다음 회차는 앵커 잡을 때 참고할 것
- 스파크라인 3초 타이머는 `observeInvestorCards` 재호출 시점에만 정리된다. 13F를 벗어나는 모든 경로를 훅하는 중앙 지점이 없어서인데, 콜백이 `el.isConnected`를 확인하므로 실행돼도 no-op이다
- 실제 hidden에서 visible로의 브라우저 전이는 자동화 탭 한계로 재현하지 못했다. 핸들러 로직만 검증
- 상세 페이지의 `.inv-profile-compare` 버튼은 `toggleSelection(..., null)`로 message를 넘기지 않아 **교체 안내가 뜨지 않는다**. 기존 제약이며 상세 헤더에 메시지 영역 자체가 없다
- WORK-LOCK.md에 락을 등록하지 않았다. 작업 시작 시 표가 비어 있음을 확인했고 단독 작업이라 락 전용 커밋을 생략했다
- 같은 날 `8530edb`의 `#skew` 수정(`.sktr-meta` min-width, `.sktr-sample` ellipsis)이 라이브에 그대로 남아 있음을 확인했다. 클로버 없음
...
**결과**:
- 기존 동적 `#feedList` 앞에 정적 최신 글 영역을 추가하고, 기존 데이터 빌드 때 `items.json` 기준 최신 10개를 자동 생성하도록 했다.
- `pages build and deployment` run `31298119426` 성공.
- `https://stacksdaily.com/` 원본 HTML 검증: 정적 카드 10개, `Why it matters` 10개, `p/en/` 내부 링크 10개, 로딩 문구 없음. 첫 기사 상세 URL HTTP 200.
- `OG card images` run `31297770258` 성공. 최종 watcher 변경에 대한 후속 run `31298071391`은 이 기록 시점에 이미지 수집 단계 진행 중이다.
- `items.json`은 직접 수정하지 않았다.

**판단**: JavaScript가 실행되기 전에도 크롤러·no-JS 환경에서 제목·요약·해석·내부 기사 탐색 링크를 읽을 수 있다. JavaScript 부팅 후에는 기존 동적 피드가 계속 렌더링되며, 첫 페인트 예약 해제는 실제 `article.card`가 생긴 뒤로 유지했다.

**남은 사항**: 이 변경은 콘텐츠 탐색성과 정적 노출을 보강하지만, AdSense의 “가치가 별로 없는 콘텐츠” 판정을 단독으로 해결하거나 승인을 보장하지 않는다. 독창적 분석, 작성자·편집 정책, 사이트 신뢰 신호는 별도 과제다.

## 2026-08-09 Codex — 홈 최신 글 정적 노출 구현

**목표**: 홈의 빈 `#feedList`를 보완해, 브라우저 JavaScript가 실행되기 전에도 최신 글 목록과 내부 기사 링크가 HTML에 존재하도록 한다. 애드센스 승인 자체를 보장하는 변경은 아니며, 크롤링 가능한 탐색·콘텐츠 표면을 보강하는 목적이다.

**변경 파일**:
- `index.html` — `#feedList` 안에 정적 홈 피드 마커·스타일·대체 콘텐츠 영역 추가.
- `scripts/build_data.py` — `items.json`에서 날짜순 최신 10개를 제목·요약·Stacks의 해석·출처·날짜·정적 기사 링크로 생성하고, 기존 자동 빌드 때 `index.html`의 마커 영역만 갱신. `items.json`은 직접 수정하지 않는다.

**검증**:
- `build_data.py` AST 문법 검사 통과.
- `static_home_block` 샘플 self-check 통과: HTML 이스케이프, 비-HTTP 출처 URL 차단, 마커·카드 수 확인.
- `index.html`에서 정적 영역의 시작·끝 마커와 `#feedList` 위치 확인.
- main 대비 변경 파일 2개, 추가 113줄·삭제 2줄.

**위험/다음**:
- 정적 목록은 기본 영문 요약을 사용하고, 브라우저가 부팅되면 기존 동적 피드가 같은 `#feedList`를 다시 그린다.
- PR 병합 후 OG/정적 빌드와 Pages 배포 성공을 확인하고, 라이브 HTML에서 최신 카드 제목·`p/en/` 내부 링크가 노출되는지 검증한다.

## 2026-08-08 Codex — 테마 UX·분류 재빌드 배포

**변경**: 테마 카드 라벨·정렬·상세 페이지네이션·데스크톱 폭·모바일 푸터를 개선하고,
`rates` 오탐 규칙을 줄였다. 원격 최신 `items.json` 기준으로 데이터와 정적 테마/기록 허브를 재생성했다.

**검증**: 289개 항목, 13개 청크, JSON/JS/Python 문법 확인. IndexNow 외부 ping은 실행하지 않았다.

**위험**: GitHub Pages 배포 전 원격 최신 커밋에 리베이스했다.
...
`semianalysis`로 통일했다. 유료 항목에는 `유료글`/언어별 paywall 라벨을 표시하고,
공개된 일부 원문을 인용해 작성했다는 고지를 인용문·카드·상세 CTA·정적 페이지에
추가했다.

**검증**: 인라인 JS 문법, Python/JSON 문법, 프런트 계약 테스트 9개, Playwright
회귀 테스트 13개, 유료 인용 고지 렌더링, `git diff --check` 통과.
