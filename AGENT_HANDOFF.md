...
터치 스모크 테스트는 미실행. 시세가 없는 선택 구간은 `—`로 남긴다. 코드 변경은 로컬
작업 브랜치에만 있으며 production `main`에는 아직 반영하지 않았다.

**다음**: 사용자 확인 후 최신 `main` 기준 충돌을 재확인하고 production에 반영한 뒤,
실제 비교 화면에서 상단 배치·드래그 수익률·모바일 터치 동작을 확인한다.
