...
class FrontendContracts(unittest.TestCase):
    ...
    def test_all_detail_paths_use_the_card_factory(self):
        ...
        self.assertIn("if (!_ac) _ac = cardEl(_it, S, 0, true);", detail)
    ...
    def test_home_defers_source_quotes_until_article_open(self):
        self.assertIn("function srcBlockHtml(item, full)", INDEX)
        self.assertIn("<div data-source-shell>", INDEX)
        self.assertIn("srcBlockHtml(item, !!detail)", INDEX)
        self.assertIn("window.expandArticleSource", INDEX)
        self.assertIn("expandArticleSource(card)", V82)

    def test_english_article_indexing_hub_contracts(self):
        pages = (ROOT / "scripts" / "build_pages.py").read_text(encoding="utf-8")
        self.assertIn('ARTICLE_HUB_FILE = {"ko": "articles.html", "en": "articles-en.html"', pages)
        self.assertIn('meta name="robots" content="index,follow,max-image-preview:large', pages)
        self.assertIn('BASE + "articles-en.html"', pages)
        self.assertIn('articles_index(items, "en")', pages)
        self.assertTrue((ROOT / "articles-en.html").exists())
    ...
    def test_static_pages_have_main_landmarks(self):
        pages = (ROOT / "scripts" / "build_pages.py").read_text(encoding="utf-8")
        self.assertEqual(pages.count("<main>"), 7)
        self.assertEqual(pages.count("</main>"), 7)
        for path in (
            ROOT / "about.html",
            ROOT / "privacy.html",
